...
interface StudentDetailModalProps {
  ...
  studentBookmarkedLessonIds: string[];
  studentCompletedHomeworkIds?: string[];
...
export const StudentDetailModal: React.FC<StudentDetailModalProps> = ({
  ...
  allHomeworks,
  allSubmissions,
  studentBookmarkedLessonIds = [],
  studentCompletedHomeworkIds = [],
  onSelectLesson
}) => {
  const [activeTab, setActiveTab] = useState<'bookmarks' | 'homeworks'>('bookmarks');
  ...
  const submissionsWithHomework = studentSubmissions.map((sub) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 bg-slate-900/60 backdrop-blur-xs font-['Tajawal',sans-serif]">
      ...
      >
        ...
        <div className="p-4 sm:p-5 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white relative">
          ...
          <div className="grid grid-cols-2 gap-2 mt-4 pt-3 border-t border-white/10">
            <div className="bg-white/10 rounded-xl p-2.5 flex items-center gap-2.5 backdrop-blur-xs">
              ...
              <div>
                <span className="text-[10px] text-slate-300 block">المحفوظات</span>
                <span className="text-xs sm:text-sm font-black text-white">
                  {bookmarkedLessons.length} درس محفوظ
                </span>
              </div>
            </div>
        ...
        {/* Tab Switcher */}
        <div className="flex items-center border-b border-slate-100 bg-slate-50/80 p-1.5 gap-1.5 px-4">
          <button
          ...
          >
            <Bookmark className="w-4 h-4" />
            <span>المفضلات ({bookmarkedLessons.length})</span>
          </button>
          ...
          >
            <ClipboardList className="w-4 h-4" />
            <span>حلول الواجبات ({studentSubmissions.length})</span>
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3">
          {activeTab === 'bookmarks' ? (
            /* TAB 1: Bookmarks / Favorites */
            <div className="space-y-2.5">
              {bookmarkedLessons.length > 0 ? (
                bookmarkedLessons.map((lesson) => {
                  const sub = allSubjects.find((s) => s.id === lesson.subjectId);
                  const isDownloading = downloadingFileId === (lesson.attachedFile?.fileId || lesson.id);

                  return (
                    <div
                      key={lesson.id}
                      className="bg-white border border-slate-200/80 rounded-2xl p-3.5 hover:border-indigo-300 transition flex items-center justify-between gap-3 shadow-2xs group"
                    >
                      <div className="min-w-0 flex-1 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          {sub && (
                            <span className="text-[10px] px-2 py-0.5 rounded-md font-bold bg-blue-50 text-blue-700 border border-blue-100">
                              {sub.emoji} {sub.name}
                            </span>
                          )}
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        {lesson.attachedFile?.hasFile && (
                          <button
                            onClick={() =>
                              handleDownloadPdf(
                                lesson.attachedFile?.fileId,
                                lesson.attachedFile?.dataUrl,
                                lesson.attachedFile?.name || `${lesson.title}.pdf`
                              )
                            }
                            disabled={isDownloading}
                            className="py-1.5 px-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200/70 rounded-xl text-[11px] font-bold transition flex items-center gap-1 cursor-pointer disabled:opacity-50"
                            title="تحميل ملف الدرس PDF"
                          >
            ...
            /* TAB 2: Homework Solutions / Submissions */
