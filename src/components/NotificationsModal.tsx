...
export const NotificationsModal: React.FC = () => {
  ...
  // If teacher, they see their own notifications ("إشعاراتي المرسلة") as requested: "و خلي كل استاذ يقدر يشوف إشعاراته فقط"
  // If supervisor, they can toggle or see all notifications
  // If student, they see all active teacher & supervisor notifications
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('هل أنت متأكد من حذف هذا الإشعار؟ لن يتمكن الطلاب من رؤيته بعد الحذف.')) {
      return;
    }
    setDeletingId(id);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 bg-slate-900/60 backdrop-blur-xs font-['Tajawal',sans-serif]">
      ...
      >
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                {isTeacher
                  ...
                  : 'آخر تنبيهات وتوجيهات معلمي المقررات الدراسية'}
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            {/* Add notification button for teachers and supervisors */}
            {(isTeacher || isSupervisor) && (
              <motion.button
                ...
                className={`px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition shadow-xs cursor-pointer ${
                  isAddingNew
                    ? 'bg-slate-200 text-slate-700 hover:bg-slate-300'
                    : 'bg-indigo-600 hover:bg-indigo-700 text-white'
                }`}
                title={isAddingNew ? 'إلغاء الإضافة' : 'إرسال إشعار جديد'}
              >
                {isAddingNew ? (
                  <>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          {/* Add Notification Form (For Teachers / Supervisors) */}
          <AnimatePresence>
            {isAddingNew && (
              <motion.div
              ...
              >
                <div className="bg-indigo-50/60 border border-indigo-200/90 rounded-2xl p-4 sm:p-5 space-y-3.5 shadow-xs">
                  ...
                  <form onSubmit={handleSendSubmit} className="space-y-3">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-1">
                        المقرر الدراسي المرتبط بالإشعار
                      </label>
                      {isSupervisor ? (
                        <select
                        ...
                        >
                          ...
                          {INITIAL_SUBJECTS.map((s) => (
                            <option key={s.id} value={s.id}>
                              {s.emoji} {s.name} (P{s.semester})
                            </option>
                          ))}
                        </select>
                      ) : (
                        <div className="w-full py-2 px-3 bg-white border border-indigo-200 rounded-xl text-xs font-black text-indigo-800 flex items-center justify-between shadow-2xs">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="مثال: تنبيه هام بخصوص موعد الواجب أو الاختبار القادم"
                        className="w-full py-2 px-3 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-2xs"
                        maxLength={100}
                        required
                      />
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="اكتب التوجيهات أو الملاحظات التي ترغب بإيصالها لطلابك فوراً..."
                        rows={3}
                        className="w-full py-2 px-3 bg-white border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-2xs resize-none"
                        maxLength={500}
                        required
                      />
                    ...
                    <div className="flex justify-end gap-2 pt-1">
                      <motion.button
                        ...
                        className="w-full py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold flex items-center justify-center gap-2 shadow-xs transition cursor-pointer disabled:opacity-50"
                      >
                        <Send className="w-4 h-4" />
                        <span>{isSubmitting ? 'جاري الإرسال...' : 'إرسال الإشعار فوراً للطلاب'}</span>
                      </motion.button>
                    </div>
                  </form>
                </div>
          ...
          {displayedList.length === 0 ? (
            <div className="text-center py-12 px-4 bg-slate-50/60 rounded-3xl border border-dashed border-slate-200 space-y-3">
              ...
              <p className="text-xs text-slate-400 max-w-xs mx-auto">
                {isTeacher
                  ? 'اضغط على زر (إضافة إشعار) في الأعلى لإرسال تنبيه فوري ومباشر لطلاب مادتك.'
                  : 'سيتم إشعارك فور قيام المعلمين بنشر تنبيهات وتوجيهات دراسية جديدة.'}
              </p>
            </div>
          ) : (
              ...
              {displayedList.map((notif) => {
                ...
                return (
                  <motion.div
                    ...
                    className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-2xs hover:shadow-xs transition space-y-2.5 relative group"
                  >
                    {/* Top Row: Subject Badge & Date & Actions */}
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {/* Author info if student/supervisor */}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[11px] text-slate-400 font-medium flex items-center gap-1" dir="ltr">
                          <span>{formatArabicDate(notif.createdAt)}</span>
                          <Calendar className="w-3 h-3 text-slate-400" />
                        </span>

                        {/* Delete Button for author or supervisor */}
                        {canDelete && (
                          <button
                            type="button"
                    ...
                    {/* Message Body */}
                    <p className="text-xs sm:text-sm text-slate-600 leading-relaxed whitespace-pre-line bg-slate-50/50 p-2.5 rounded-xl border border-slate-100">
                      {notif.message}
                    </p>

                    {/* Attached Link Button */}
                    {notif.linkUrl && (
                      <div className="pt-1">
                        <a
                          href={notif.linkUrl}
                          ...
                          className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 hover:text-indigo-900 border border-indigo-200/90 text-xs font-black transition-all shadow-2xs cursor-pointer group/link active:scale-98"
                          title={notif.linkUrl}
                        >
                          <LinkIcon className="w-3.5 h-3.5 text-indigo-600 group-hover/link:rotate-45 transition-transform" />
                          <span>فتح الرابط المرفق</span>
                          <ExternalLink className="w-3 h-3 text-indigo-500 mr-0.5" />
                        </a>
                      </div>
                    )}
                  </motion.div>
